...
## 权限访问验证，相关页面放在 app/pages/(permission) 分组
...
上层角色：管理员         app/pages/(permission)/(admin)                   存放当前角色访问页面 以及 下层角色分组
下层角色：财务部         app/pages/(permission)/(admin)/(finance)         存放当前角色访问页面 以及 下层角色分组
下层角色：人事部         app/pages/(permission)/(admin)/(hr)              存放当前角色访问页面 以及 下层角色分组
下层角色：技术部         app/pages/(permission)/(admin)/(technical)       存放当前角色访问页面 以及 下层角色分组
角色共享（人事部与技术部）：app/pages/(permission)/(admin)/(hr_technical)   存放对应角色共享页面，命名规则：以下划线区分，角色A_角色N
```
